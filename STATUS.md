...
基本エフェクトを単一機能ずつ追加中です。初版機能とFade、Peek、Wipe、Zoom基本版は完成・検証済みです。4エフェクト共通のイージング基本機能とWipeのSoftnessも自動試験と実OBS目視確認に合格しました。Media Source非表示境界の1フレーム混入修正は自動試験・実OBS目視確認ともに合格しました。Zoom倍率UIは通常step 0.1%・高倍率step 10%の最終仕様です。
...
- Zoom倍率UI：通常は0〜500%・step 0.1%のOBS標準スライダー一つ。開始／終了独立のチェックでstep 10%の高倍率数値入力へ切替
